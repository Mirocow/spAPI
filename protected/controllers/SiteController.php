...
class SiteController extends Controller
{
    ...
    public function actionClaims($guid = null, $render = true)
    {
        ...
        foreach($claims as $claim)
            $response[] = array('id' => $claim->id, 'name' => $claim->name, 'error' => $claim->error, 'guid' => $claim->guid, 'status' => $claim->status, 'class' => $claim->getClass(), 'statusText' => $claim->getStatusText(), 'errorText' => $claim->getErrorText(), 'created' => date('H:i d.m.Y', strtotime($claim->created)), 'closed' => date('H:i d.m.Y', strtotime($claim->closed)), 'comment' => $claim->comment);
    ...
    public function actionNewClaim($guid)
    {
        ...
        if($guid)
        {
            if($data !== array())
            {
                ...
                $claim->save();
                CVarDumper::dump($claim->errors(),100,false);
            }
        }
    }
